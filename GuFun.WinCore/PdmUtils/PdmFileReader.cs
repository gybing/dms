...
namespace GuFun.WinCore.PdmUtils
{
    public class PdmFileReader
    {
            ...
            foreach (XmlNode xnP in xnCProperty)
            {
                switch (xnP.Name)
                {
                    ...
                    case "a:Identity": mColumn.Identity = ConvertToBooleanPG(xnP.InnerText);
                        break;
                    case "a:Column.Mandatory": mColumn.Mandatory = ConvertToBooleanPG(xnP.InnerText);
                        break;
        ...
        private void InitKeyColumns(XmlNode xnKeyColumns, PdmKey1 Key)
        {
            XmlElement xe = (XmlElement)xnKeyColumns;
            XmlNodeList xnKProperty = xe.ChildNodes;
            IList<ColumnInfo> columninfos = Key._OwnerTable.Columns;
            foreach (XmlNode xnP in xnKProperty)
            {
                string theRef = ((XmlElement)xnP).GetAttribute("Ref");
                foreach (ColumnInfo columninfo in columninfos)
                {
                    string columnid = columninfo.ColumnId;
                    if (theRef.Equals(columnid))
                    {
                        Key.AddColumnObjCode(columninfo.Code);
                    }
                }

            }
        }
        private PdmKey1 GetKey(XmlNode xnKey, TableInfo OwnerTable)
        {
            PdmKey1 mKey = new PdmKey1(OwnerTable);
            XmlElement xe = (XmlElement)xnKey;
