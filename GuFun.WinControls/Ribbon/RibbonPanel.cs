...
namespace GuFun.WinControls
{
      ...
      Component, IRibbonElement, IContainsSelectableRibbonItems, IContainsRibbonComponents
  {
    ...
    /// </summary>
    public event MouseEventHandler Paint;
    ...
    internal void SetSelected(bool selected, int mouseX, int mouseY)
    {
      _selected = selected;

      if (selected)
      {
        OnMouseEnter(new MouseEventArgs(MouseButtons.None, 0, mouseX, mouseY, 0));
      }
      else
      {
        OnMouseLeave(new MouseEventArgs(MouseButtons.None, 0, mouseX, mouseY, 0));
      }
    }
