  ...
  public class IPTextBox : TextBox
  {
    ...
    public override string Text
    {
      ...
      set
      {
        try
        {
          if (IPTextBox.ValidateIP(value, this.m_newIPNotation, this.m_arlDelimeter))
            base.Text = IPTextBox.MakeValidSpaces(value, this.m_newIPNotation, this.m_arlDelimeter);
        }
        catch
        {
    ...
    protected override void OnKeyUp(KeyEventArgs e)
    {
      ...
      string[] aIps = this.Text.Split('.');
      for (int i = 0; i < aIps.Length; i++)
      {
        if ((aIps[i][0] != ' ') && (aIps[i][2] != ' ') && (aIps[i][1] == ' '))
          aIps[i] = aIps[i][0].ToString() + "  ";
      ...
      if (aIps.Length == 4)
        this.Text = aIps[0] + aIps[1] + aIps[2] + aIps[3];

      //Cursor hintern Punkt setzen
      if ((char.IsLetterOrDigit(Convert.ToChar(e.KeyValue)) || e.KeyCode == Keys.NumPad0) 
        && iPos < this.TextLength)
      {
        if (this.m_arlDelimeter.Contains(cText[iPos]))
        {
    ...
    public string GetPureIPAddress()
    {
      ...
      s = IPTextBox.MakeIP((string[])arlIP.ToArray(typeof(string)), this.m_ipNotation);
    ...
    {
      if (IPTextBox.ValidateIP(this.Text, this.m_newIPNotation, this.m_arlDelimeter))
        return true;
      else
    ...
    protected static string MakeValidSpaces(string arg_sIP, IPNotation arg_ipNotation, ArrayList arg_arlDelimeter)
    {
      ...
      return IPTextBox.MakeIP((string[])arlIP.ToArray(typeof(string)), arg_ipNotation);
    }
    ...
    private void JumpToNowDot()
    {
      int iSelStart = this.Text.LastIndexOf('.', this.SelectionStart);
      ...
      {
        iSelStart = this.Text.LastIndexOf(':', this.SelectionStart);
        if (iSelStart >= -1)
        {
          DisplayFormatText();
          this.Select(iSelStart + 1, 3);
    ...
    private void DisplayFormatText()
    {
      string[] aIps = this.Text.Split('.');
      for (int i = 0; i < aIps.Length; i++)
      {
      ...
      if (aIps.Length == 4)
        this.Text = aIps[0].Replace(" ", "") + aIps[1].Replace(" ", "") + aIps[2].Replace(" ", "") + aIps[3].Replace(" ", "");
    }
